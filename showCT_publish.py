...
def compute_region(labels_mask, lungs_seg, imgs_mask,  metadata, save_path):
    ...
    for i in range(label_num):
        ...
        save_csv.data_write_csv(save_path + f'/label_{parameters_lung.id_name[i]}.csv', valid_label)  # save lesions V 2 Lung
...
def LabCount(data_list):
    for i, names in enumerate(data_list):
        if not names[1]:
            continue
        save_root = './LabCount_sever/' + names[0][31:]
        if not os.path.exists(save_root):
            os.makedirs(save_root)
        pices_num = len(names[1])
        labels = np.zeros([pices_num, 512, 512])
        nii_root = names[0].replace('/public/home/hedh/data/LungCT_np/Classification/melioidosis/data200_1000', '/public/home/hedh/data/Dataset_LungCT')
        ...
        for k in range(pices_num):
            ...
            lab_name = name.replace('data200_1000', 'label200_1000_lesion3')
            lung_root = name.replace('Classification/', 'pneumonie_np_lungseg/')
...
if __name__ == '__main__':
    ...
    data_list = []
    lung_lab_list = []
    for root, dirs, files in os.walk('/public/home/hedh/data/LungCT_np/Classification/melioidosis/data200_1000/'):
        names = []
        if '1.2.840.113619.2.334.3.17475087.259.1415686362.551.3' in root:
            for name in files:
                if 'npy' in name:
                        names.append(name)
